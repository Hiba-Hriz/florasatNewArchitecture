...
{
    const LoRaTransmission *loRaTransmission =
        check_and_cast<const LoRaTransmission *>(transmission);

    if (iAmGateway)
        return true;

    auto *loRaApp = check_and_cast<SimpleLoRaApp *>(
        getParentModule()->getParentModule()->getSubmodule("SimpleLoRaApp"));


    auto *loRaMac = check_and_cast<LoRaMac *>(
        getParentModule()->getParentModule()->getSubmodule("mac"));

    if (loRaTransmission->getLoRaCF() == inet::Hz(869525000)
        && loRaMac->isInBeaconReception())
        return loRaTransmission->getLoRaBW() == loRaApp->loRaBW;

    // Normal uplink/downlink: all three must match
    return (loRaTransmission->getLoRaCF() == loRaApp->loRaCF
         && loRaTransmission->getLoRaBW() == loRaApp->loRaBW
         && loRaTransmission->getLoRaSF() == loRaApp->loRaSF);
}

bool LoRaReceiver::computeIsReceptionPossible(const IListening *listening,
    const IReception *reception, IRadioSignal::SignalPart part) const
{
    const LoRaBandListening *loRaListening =
        check_and_cast<const LoRaBandListening *>(listening);
    const LoRaReception *loRaReception =
        check_and_cast<const LoRaReception *>(reception);

    if (iAmGateway == false)
    {
        // Beacon bypass: only when MAC is actually in beacon reception
        if (loRaReception->getLoRaCF() == inet::Hz(869525000))
        {
            auto *loRaMac = check_and_cast<LoRaMac *>(
                getParentModule()->getParentModule()->getSubmodule("mac"));

            if (loRaMac->isInBeaconReception())
            {
                // Only check power, skip SF/CF filter
                W minReceptionPower = loRaReception->computeMinPower(
                    reception->getStartTime(part), reception->getEndTime(part));
            ...
            // Not in beacon reception: fall through to normal check
        }

        // Normal check: CF, BW, SF must all match listening parameters
        if (loRaListening->getLoRaCF() != loRaReception->getLoRaCF() ||
            loRaListening->getLoRaBW() != loRaReception->getLoRaBW() ||
    ...
    EV << "Computing whether reception is possible: minimum reception power = "
       << minReceptionPower << ", sensitivity = " << sensitivity
       << " -> reception is " << (isReceptionPossible ? "possible" : "impossible") << endl;
    ...
    const Coord &startPosition, const Coord &endPosition) const
{
    if (iAmGateway == false)
    {
        ...
        auto loRaMac = check_and_cast<LoRaMac *>(node->getSubmodule("LoRaNic")->getSubmodule("mac"));

        Hz listeningCF = loRaApp->loRaCF;
        int listeningSF = loRaApp->loRaSF;

        // Only use beacon SF when MAC is actually in beacon reception state
        if (listeningCF == inet::Hz(869525000) && loRaMac->isInBeaconReception())
            listeningSF = 10;

        return new LoRaBandListening(radio, startTime, endTime, startPosition,
                                     endPosition, listeningCF, loRaApp->loRaBW, listeningSF);
    }
    else
        return new LoRaBandListening(radio, startTime, endTime, startPosition,
                                     endPosition, LoRaCF, LoRaBW, LoRaSF);
}
